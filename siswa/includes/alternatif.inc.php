...
class Alternatif{
	...
	function readAll11(){
		...
		$query = "SELECT * FROM vektor INNER JOIN user ON vektor.siswa=user.id INNER JOIN wp_alternatif ON vektor.alternatif=wp_alternatif.id_alternatif WHERE vektor.siswa='$_SESSION[id]' GROUP BY vektor.vektor_v DESC ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function readAll2(){
		...
		// $query = "SELECT DISTINCT vektor.alternatif FROM vektor INNER JOIN user ON vektor.siswa=user.id INNER JOIN wp_alternatif ON vektor.alternatif=wp_alternatif.id_alternatif WHERE vektor.siswa='$_SESSION[id]' GROUP BY vektor.id DESC";
		$query = "SELECT * FROM vektor INNER JOIN user ON vektor.siswa=user.id INNER JOIN wp_alternatif ON vektor.alternatif=wp_alternatif.id_alternatif WHERE vektor.siswa='$_SESSION[id]' GROUP BY vektor.vektor_v DESC ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
